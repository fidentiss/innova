...
const QString BaseURL4 = "http://innovacoin.io/inneur.php";
double eurx;
double innovax;
double innbtcx;
...
if (what == BaseURL3) // Innova News Feed
{
    ...
    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString innnewsfeed = finished->readAll();
    //innnewsfeedx = (innnewsfeed.toDouble());
    //innnewsfeed = QString::number(innnewsfeedx, 'f', 8);

}
if (what == BaseURL4) // Innova EUR Price
{

    QJsonDocument doc = QJsonDocument::fromJson(finished->readAll());
    QJsonObject jObj = doc.object();
    QJsonObject rateObj = jObj["rates"].toObject();
    QString eur = rateObj["EUR"].toString();
    eurx = (eur.toDouble()) * dollarg.toDouble();
    eur = QString::number(eurx, 'f', 8);
